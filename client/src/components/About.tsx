...
export default function About() {
  return (
    <div className="w-full grid lg:grid-cols-2 gap-10 md:gap-16 items-center">
      <motion.div
        className="space-y-7"
        ...
        viewport={{ once: true }}
      >
        <p className="font-mono text-xs uppercase tracking-[0.3em] text-primary">About Hypertech</p>
        ...
        <div className="space-y-5 text-white/70 leading-relaxed text-base md:text-lg font-light max-w-2xl">
          <p>
            Great websites are more than visuals. They should communicate trust in seconds, remove friction from every step, and guide visitors to clear decisions. That is the standard I build for.
          </p>
          <p>
            From landing pages to full-scale platforms, I deliver end-to-end solutions with strong performance, premium interaction design, and persuasive copy that helps your business win better clients.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4 pt-3">
          {milestones.map((item) => (
            ...
            >
              <h4 className="text-3xl font-black font-display text-white mb-2">{item.value}</h4>
              <p className="text-[11px] text-white/45 font-mono uppercase tracking-[0.22em]">{item.label}</p>
            </motion.div>
          ))}
        </div>
      </motion.div>
      ...
      >
        <div className="aspect-[4/5] w-full max-w-md mx-auto rounded-[2rem] overflow-hidden relative shadow-2xl border border-white/10">
          <img src="/images/profile.jpg" alt="Hypertech developer portrait" className="w-full h-full object-cover" />
        ...
        <div className="absolute bottom-6 left-3 right-3 sm:left-8 sm:right-auto sm:max-w-[75%] p-5 rounded-2xl bg-black/65 border border-white/15 backdrop-blur-xl">
          <p className="text-white text-sm leading-relaxed">
            “Every project starts with one goal: make your audience feel they have found the right team before they even hit contact.”
          </p>
        </div>
