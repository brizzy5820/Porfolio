...
export default function Sidebar({ activeSection, setActiveSection }: SidebarProps) {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-2 lg:w-58 border-r border-white/5 bg-white/5 backdrop-blur-xl h-screen sticky top-0 flex-col items-center lg:items-start py-8 z-40 transition-all duration-300 shrink-0">
        <div className="lg:px-8 mb-12 flex flex-col items-center lg:items-start w-full">
          ...
          <div className="lg:hidden text-primary font-bold text-2xl tracking-tighter flex items-center justify-center w-10 h-10 rounded bg-white/5">
            H
          </div>
        </div>
            ...
            return (
              <li key={item.id} className="w-full flex justify-center lg:block">
                ...
                >
                  ...
                  <span className="hidden lg:block relative z-10 font-medium tracking-wide">
                    {item.label}
                  </span>
                </button>
              </li>
